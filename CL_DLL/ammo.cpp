...
void WeaponsResource::SelectSlot (int iSlot, int fAdvance, int iDirection)
	...
	bool fastSwitch = true;	// ѕринудительно
	...
	if ((gpActiveSel == NULL) || (gpActiveSel == (WEAPON*)1) || (iSlot != gpActiveSel->iSlot))
		...
		/* Ёта часть перестроена под сверхбыструю смену оружи€ (как в HL2) */
		WEAPON* p2, * p3 = gHUD.m_Ammo.m_pWeapon;	// ƒействующее оружие

		if (p && fastSwitch && p3) // check for fast weapon switch mode
			{
