...
// barnacle has prey.
	...
	// still pulling prey.
		...
		// prey has just been lifted into position ( if the victim origin + eye height + 8 is higher than the bottom of the barnacle, it is assumed that the head is within barnacle's body )
				...
				EMIT_SOUND( ENT(pev), CHAN_WEAPON, "barnacle/bcl_bite3.wav", 1, ATTN_MEDIUM );	
...
// barnacle has no prey right now, so just idle and check to see if anything is touching the tongue.
		...
		if ( pTouchEnt != NULL && m_fTongueExtended )
		{
			...
			{
				EMIT_SOUND( ENT(pev), CHAN_WEAPON, "barnacle/bcl_alert2.wav", 1, ATTN_MEDIUM );	
...
//	CGib::SpawnRandomGibs( pev, 4, 1 );
	...
	case 1:	EMIT_SOUND( ENT(pev), CHAN_WEAPON, "barnacle/bcl_die3.wav", 1, ATTN_MEDIUM );	break;
	}
