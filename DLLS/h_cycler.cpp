...
// Контроль корректности задания параметров
...
// Поддержка возможности задания физического размера модели
...
LINK_ENTITY_TO_CLASS (env_model, CGenericCycler);	// Совместимость с AOMDC
...
void CCycler :: Spawn ()
	...
	m_flFrameRate		= 75;
	m_flGroundSpeed		= 0;
	m_bloodColor		= DONT_BLEED;	// Странно, когда, например, деревья брызгают жёлтой кровью

	pev->nextthink		+= 1.0;

	ResetSequenceInfo ();

	if (/*pev->sequence != 0 ||*/ pev->frame != 0)	// Непонятно, зачем было отключать анимацию на ненулевых секъюэнсах
		{
		m_animate = 0;
		pev->framerate = 0;
		}
...
void CCycler :: DamageSound (void)		// Скопировано из CBreakable
	{
	int pitch;
	float fvol;
	char *rgpsz[6];
