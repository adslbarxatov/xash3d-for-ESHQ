...
void CHGrunt :: GibMonster (void)
	...
	if (GetBodygroup (2) != 2)	// Эта зона, видимо, вообще не работает при текущих моделях
		...
		if (pGun)
			{
			// Чтобы в стенах реже застревали, уберём разброс по горизонтальным осям и расположим оружие прямо над телом
...
void CHGrunt :: PainSound (void)
	...
	if (gpGlobals->time > m_flNextPainTime)
		...
		if (RANDOM_LONG (0,99) < 5)
			...
			if (FOkToSpeak ())
				{
				SENTENCEG_PlayRndSz (ENT (pev), "HG_PAIN", HGRUNT_SENTENCE_VOLUME, ATTN_MEDIUM, 0, PITCH_NORM);
				JustSpoke ();
				return;
				}
			}
