...
void CBaseMonster :: MonsterThink (void)
	...
	if (m_MonsterState != MONSTERSTATE_SCRIPT && m_MonsterState != MONSTERSTATE_DEAD && m_Activity == ACT_IDLE && m_fSequenceFinished)
		...
		if (iSequence != ACTIVITY_NOT_AVAILABLE)
			{
			if (!(pev->spawnflags & SF_MONSTER_OWN_SEQUENCE))	// Запрещено, если требуется специальная сцена
				{
...
void CBaseMonster :: MakeIdealYaw (Vector vecTarget)
	...
	// Запретить поворот при фиксированном sequence
...
CBaseEntity* CBaseMonster :: DropItem (char *pszItemName, const Vector &vecPos, const Vector &vecAng)
	...
	if (pItem)
		{
		pItem->pev->origin = Vector (pev->origin.x, pev->origin.y, pItem->pev->origin.z);	// Принудительное расположение над инициатором
		pItem->pev->velocity = Vector (0, 0, RANDOM_FLOAT (0, 100));						// Запрет на боковые смещения
