...
{
	// Логичнее будет, если радиус пойдёт по убывающей
	...
	{// if the designer didn't set a sound attenuation, default to small radius.
		m_flAttenuation = ATTN_SMALL;
	}
...
// hit the world, try to play sound based on texture material type
	...
	// Мы ударили breakable или cycler?
	...
	else if (chTextureType == CHAR_TEX_COMPUTER)
	{
		...
		if ( ptr->flFraction != 1.0 && RANDOM_LONG(0,1))
		{
			...
			{
				case 0: UTIL_EmitAmbientSound(ENT(0), ptr->vecEndPos, "buttons/spark5.wav", flVolume, ATTN_MEDIUM, 0, 100); break;
				case 1: UTIL_EmitAmbientSound(ENT(0), ptr->vecEndPos, "buttons/spark6.wav", flVolume, ATTN_MEDIUM, 0, 100); break;
	...
	//EMIT_SOUND_DYN( ENT(m_pPlayer->pev), CHAN_WEAPON, rgsz[RANDOM_LONG(0,cnt-1)], fvol, ATTN_MEDIUM, 0, 96 + RANDOM_LONG(0,0xf));
