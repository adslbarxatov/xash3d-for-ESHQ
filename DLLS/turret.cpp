...
void CMiniTurret::Shoot(Vector &vecSrc, Vector &vecDirToEnemy)
{
	...
	{
	case 0: EMIT_SOUND(ENT(pev), CHAN_WEAPON, "weapons/hks1.wav", 1, ATTN_MEDIUM); break;
	case 1: EMIT_SOUND(ENT(pev), CHAN_WEAPON, "weapons/hks2.wav", 1, ATTN_MEDIUM); break;
	case 2: EMIT_SOUND(ENT(pev), CHAN_WEAPON, "weapons/hks3.wav", 1, ATTN_MEDIUM); break;
	}
	pev->effects = pev->effects | EF_MUZZLEFLASH;
}
...
void CBaseTurret ::	TurretDeath( void )
{
	...
	if (pev->deadflag != DEAD_DEAD)
	{
		...
		if ( flRndSound <= 0.33 )
			EMIT_SOUND(ENT(pev), CHAN_BODY, "turret/tu_die.wav", 1.0, ATTN_MEDIUM);
		else if ( flRndSound <= 0.66 )
			EMIT_SOUND(ENT(pev), CHAN_BODY, "turret/tu_die2.wav", 1.0, ATTN_MEDIUM);
		else 
			EMIT_SOUND(ENT(pev), CHAN_BODY, "turret/tu_die3.wav", 1.0, ATTN_MEDIUM);
...
int CBaseTurret::TakeDamage(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType)
{
	...
	if (pev->health <= 0)
	{
		// Заставляем turret вызывать цель triggertarget
