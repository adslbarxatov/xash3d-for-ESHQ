...
void CL_TempEntPlaySound (TEMPENTITY* pTemp, float damp)
	...
	if (damp > 0.0f)
		...
		S_StartSound (pTemp->entity.origin, -(pTemp - cl_tempents), CHAN_BODY, handle, fvol, ATTN_MEDIUM, pitch, SND_STOP_LOOPING);
		}
	}
...
void R_RicochetSound (const vec3_t pos)
	...
	S_StartSound (pos, 0, CHAN_AUTO, handle, fvol, ATTN_MEDIUM, iPitch, 0);
	}
