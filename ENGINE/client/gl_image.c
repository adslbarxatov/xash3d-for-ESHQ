...
void GL_Bind (GLint tmu, GLenum texnum)
	...
	// 4529: missed or invalid texture?
...
static void GL_ProcessImage (gl_texture_t* tex, rgbdata_t* pic)
	...
	if (ImageDXT (pic->type))
		...
		// 4529: dedicated server doesn't register this variable
		if (gl_emboss_scale != NULL)
			emboss_scale = gl_emboss_scale->value;

		// 4529: processing image before uploading (force to rgba, make luma etc)
		//if (pic->buffer) Image_Process (&pic, 0, 0, img_flags, gl_emboss_scale->value);
		if (pic->buffer) 
			Image_Process (&pic, 0, 0, img_flags, emboss_scale);
...
void GL_FreeTexture (GLenum texnum)
	{
	// 4529: number 0 it's already freed
...
================
GL_TexMemory
4529: return size of all uploaded textures
================
*/
