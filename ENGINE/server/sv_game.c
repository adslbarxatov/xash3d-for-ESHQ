...
=================
*/
int SV_BuildSoundMsg (sizebuf_t* msg, edict_t* ent, int chan, const char* sample, int vol, 
	float attn, int flags, int pitch, const vec3_t pos)
	{
	int	entityIndex;
	...
	if (sound_idx < 0)
		{
		Con_Printf (S_ERROR "SV_StartSound: %s not precached\n", sample);
		return 0;
		}
